...
// Fixture artwork rather than a live campaign: this covers the component, which
// has to behave the same for whatever campaign is running.
...
describe("newComponents.AdContainer static creatives", () => {
  ...
  it("drops the house fallback on a directly-sold slot", async () => {
    // A directly-sold slot has no no-fill to apologise for, and the plea would
    // otherwise sit behind artwork that doesn't cover the whole reserved box.
    const slot = await render({
...
describe("newComponents.AdContainer server rendering", () => {
  // The whole point of doing this with <picture> rather than by measuring the
  // box: the creative is in the server response, so nothing pops in after
  // hydration, and the browser re-picks on resize by itself.
  ...
  it("offers each alternative shape under its own media condition", () => {
    // The wider one must be gated and the narrower one must be the ungated
    // <img> every browser can fall back to.
  ...
  it("lazy-loads the creative", () => {
    // Keeps a below-the-fold bottom row — and a rail tier the height cascade has
    // hidden — from fetching anything at all.
    expect(
      markup({
  ...
  it("declares each creative's intrinsic size", () => {
    // So the browser knows the aspect of whichever source it picks.
    const html = markup({

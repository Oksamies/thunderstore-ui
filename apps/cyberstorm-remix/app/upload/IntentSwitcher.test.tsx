...
describe("IntentSwitcher", () => {
  it("renders correctly with 'new' intent active", () => {
    ...
    expect(newButton?.className).toContain("intent-switcher__intent-button--active");
    expect(updateButton?.className).not.toContain("intent-switcher__intent-button--active");
  });

  it("renders correctly with 'update' intent active", () => {
    render(
      <IntentSwitcher intent="update" setIntent={vi.fn()} onNewIntent={vi.fn()} />
    );
    const newButton = screen.getByText("New Package").parentElement;
    const updateButton = screen.getByText("Update Package").parentElement;
    
    expect(newButton?.className).not.toContain("intent-switcher__intent-button--active");
    expect(updateButton?.className).toContain("intent-switcher__intent-button--active");
  });
  ...
  it("calls setIntent and onNewIntent when 'New Package' is clicked", () => {
    ...
    render(
      <IntentSwitcher intent="update" setIntent={setIntent} onNewIntent={onNewIntent} />
    );
  ...
  it("calls setIntent when 'Update Package' is clicked", () => {
    ...
    const onNewIntent = vi.fn();
    render(
      <IntentSwitcher intent="new" setIntent={setIntent} onNewIntent={onNewIntent} />
    );

...
export default class Game {
  ...
  private readonly _packageLoader: Loader;
  ...
  private readonly _communityIdentifier: string;
  ...
  constructor(
    ...
    this._communityIdentifier = communityIdentifier;
  }
  ...
  public setActivePlatformByStore(storePlatform: Platform) {
    const platform = this._storePlatformMetadata.find(
      (platform) => platform.storePlatform === storePlatform
    );

    if (!platform) {
      throw new Error(
        `Invalid store platform: "${storePlatform}" is not a valid platform for ${this.displayName}.`
      );
    }
  ...
  get isInstalledViaSteam(): boolean {
    return [Platform.STEAM, Platform.STEAM_DIRECT].includes(
      this._activePlatform.storePlatform
    );
  }
}

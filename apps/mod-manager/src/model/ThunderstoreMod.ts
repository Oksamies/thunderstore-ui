...
export default class ThunderstoreMod extends ThunderstoreVersion {
  ...
  public static defaultOrderComparer(
    a: ThunderstoreMod,
    b: ThunderstoreMod
  ): number {
    // Pinned mods first.
  ...
  public static parseFromThunderstoreData(data: any): ThunderstoreMod {
    ...
    mod.setDownloadUrl(data.versions[0].download_url);
    mod.setVersionNumber(new VersionNumber(data.versions[0].version_number));
    mod.setDependencies(data.versions[0].dependencies);
    mod.setFileSize(data.versions[0].file_size);
    return mod;
  }

...
export default class ManifestV2 {
  ...
  public makeSafeFromPartial(data: any): R2Error | ManifestV2 {
    ...
    if (data.author !== data.AuthorName) {
      ...
      this.setVersionNumber(
        new VersionNumber(data.Version || data.version_number)
      );
      this.setDependencies(data.Dependencies || data.dependencies || []);
      return this;
    }
    return new R2Error(
      "Manifest failed to be validated.",
      "The manifest is missing an author field.",
      "Add the author field to the manifest.json file manually."
    );
  }
  ...
  public fromJsObject(jsManifestObject: any): ManifestV2 {
    ...
    if (jsManifestObject.enabled === false) {
      this.disable();
    }
    return this;
  }
